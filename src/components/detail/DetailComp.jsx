...
const DetailComp = ({productDetail}) => {
  ...
  return (
    <div className='flex gap-10 my-10'>
        <img className='w-[600px] h-[600px] object-over' src={productDetail?.image} alt="" />
        <div className=''>
            ...
            <div className='text-2xl my-2 font-bold'>{productDetail?.price} <span> TL</span></div>
            <div className='flex items-center gap-5 my-4'>
                <div onClick={decrement} className='text-5xl cursor-pointer'>-</div>
                <input className='flex w-12 text-center text-4xl font-bold' type="text" value={quantity} />
                <div onClick={increment} className='text-4xl cursor-pointer'>+</div>
            </div>
            <div onClick={addBasket} className='my-4 border w-[200px] h-16 text-2xl rounded-md bg-gray-200 cursor-pointer flex items-center justify-center'>Sepete Ekle</div>
        </div>
    </div>
  )
}
